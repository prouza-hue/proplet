...
-- Raises only the Prague-day cap from 20 XP to 50 XP.
-- The +1 XP per valid side word and 5 XP per-board cap stay unchanged.
-- Replaces the existing atomic RPC without changing its signature.
...
comment on function public.proplet_claim_word_discovery(uuid, text, text) is
  'Atomically awards +1 XP for a valid side word, capped at 5 XP per board and 50 XP per Prague day.';
