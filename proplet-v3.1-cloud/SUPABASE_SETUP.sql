-- Proplet v3 Cloud – spusť CELÝ tento soubor v Supabase > SQL Editor > Run.
...
create index if not exists idx_results_player
    on public.results (player_id);
create index if not exists idx_results_daily
    on public.results (daily_date, challenge_key);

-- Data z browseru do těchto tabulek nepouštíme přímo.
-- Přístup má pouze náš FastAPI backend přes Supabase Secret key.
