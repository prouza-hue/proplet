...
function startGame(puzzle,mode,dailyDate,options={}){
 ...
 currentGame={puzzle,mode,dailyDate,found,used,path:[],dragging:false,lastPointer:null,moves:restored?.moves||0,start:performance.now(),pausedAt:null,pauseReason:null,baseElapsedMs,elapsedMs:baseElapsedMs,finished:false,lastFound:[],hints:restored?.hints||0,wrongAttempts:restored?.wrongAttempts||0,maxHintLevel:restored?.maxHintLevel||0,cleanSolve:(restored?.hints||0)===0,attemptId:restored?.attemptId||newAttemptId(),rescueFinished:false,rescueTotalLimitMs:totalLimit,rescueOffsetMs:baseElapsedMs,lastAutosaveAt:Date.now(),lastProgressAt:performance.now(),helperOffered:!!restored?.helperOffered,helperHintUsed:!!restored?.helperHintUsed,nextHintSource:'manual',isReplay:!!getState().completed[challengeKey(mode,puzzle,dailyDate)]};
 ...
 $('#timer').textContent=mode==='rescue'?fmtCountdown(remaining):fmtTime(baseElapsedMs);message(restored?'Pokračuješ přesně tam, kde jsi skončil.':'Najdi slova. A pamatuj: nestačí slovo, musí sedět i cesta.');nav('game');renderGameBoard();renderGameHUD();startTimer();if(mode!=='rescue')saveGameProgress();startAttemptTelemetry(currentGame).then(()=>{if(restored)sendAttemptCheckpoint('resume')});
}
...
function shouldOfferAccountNudge(){
 if(getProfile()?.token||currentGame?.mode==='rescue'||currentGame?.justCompleted!==true)return 0;
...
function showDailyResult(date,rec){
 ...
 $('#newBadgeBox').classList.add('hidden');configureWinReplay('daily',date,rec);$('#winShareBtn').classList.remove('hidden');$('#winMenuBtn').classList.remove('hidden');$('#winMenuBtn').textContent='Zpět na dnešek';$('#winPrimaryBtn').textContent='Vybrat další hru';$('#winModal').classList.remove('hidden');renderWinFeedback();loadWinDailyGlobalLeaderboard(date,rec);
}
function shareText(){
...
async function saveNewProfile(){
 ...
 try{
  ...
  trackProductEvent('account_authenticated');if(profileModalFromNudge&&accountNudgeStage)trackProductEvent(`account_nudge_${accountNudgeStage}_authenticated`);$('#profileModal').classList.add('hidden');await syncQueue({announce:true});updateProfileChip();renderProfile();renderDaily();renderFree();renderLeaderboard();if(profileModalFromNudge)resumeAfterAccountNudge();
...
async function maybeOpenQaDashboard(){
 ...
 try{
  ...
  <section class="card qa-section"><span class="eyebrow">ANONYMNÍ FUNNEL</span><h2>První kontakt s Propletem</h2><div class="qa-mini"><span><b>${funnel.app_open||0}</b><small>otevřelo appku</small></span><span><b>${funnel.onboarding_started||0}</b><small>začalo tutorial</small></span><span><b>${funnel.onboarding_completed||0}</b><small>dokončilo tutorial</small></span><span><b>${funnel.account_authenticated||0}</b><small>přihlásilo účet</small></span></div><p class="muted compact">Účet celkem: nabídka ${funnel.account_nudge_shown||0} · autentizace ${funnel.account_authenticated||0}<br>1. nabídka ${funnel.account_nudge_1_shown||0} → ${funnel.account_nudge_1_authenticated||0} účtů · 2. ${funnel.account_nudge_2_shown||0} → ${funnel.account_nudge_2_authenticated||0} · 3. ${funnel.account_nudge_3_shown||0} → ${funnel.account_nudge_3_authenticated||0}</p></section>
...
const ONBOARD_STEPS=[
 {title:'Najdi PES',interactive:true,html:`<div class="onboard-content"><span class="eyebrow">ZAČNI ROVNOU HRÁT</span><h2>Najdi PES</h2><p class="muted">Táhni prstem přes <b>P → E → S</b>. Jen přes políčka vedle sebe.</p><div class="tutorial-wrap"><div id="tutorialBoard" class="tutorial-board"><div class="tutorial-cell" data-tidx="0">P</div><div class="tutorial-cell" data-tidx="1">E</div><div class="tutorial-cell" data-tidx="2">L</div><div class="tutorial-cell" data-tidx="3">A</div><div class="tutorial-cell" data-tidx="4">S</div><div class="tutorial-cell" data-tidx="5">K</div><div class="tutorial-cell" data-tidx="6">M</div><div class="tutorial-cell" data-tidx="7">O</div><div class="tutorial-cell" data-tidx="8">C</div></div><div id="tutorialSuccess" class="tutorial-success"></div></div></div>`},
 {title:'Propleť všechno',html:`<div class="onboard-content"><span class="eyebrow">CELÝ PRINCIP HRY</span><h2>Propleť úplně všechno</h2><div class="onboard-fill-demo" aria-hidden="true"><span style="--d:0;--c:#ff9585">P</span><span style="--d:1;--c:#ff9585">E</span><span style="--d:2;--c:#68cfaa">L</span><span style="--d:3;--c:#7ca8ff">A</span><span style="--d:4;--c:#ff9585">S</span><span style="--d:5;--c:#68cfaa">K</span><span style="--d:6;--c:#7ca8ff">M</span><span style="--d:7;--c:#7ca8ff">O</span><span style="--d:8;--c:#68cfaa">C</span></div><p class="muted"><b>Každé políčko patří právě jednomu slovu.</b> Hotovo je, až nezůstane žádné volné.</p><div class="onboard-mini-rules"><span>↕️ ↔️ Bez diagonál</span><span>🎨 Celá plocha</span></div></div>`},
 {title:'Pomocník',support:true,html:()=>`<div class="onboard-content"><span class="eyebrow">KDYŽ SE ZASEKNEŠ</span><h2>Kdy se má Pomocník ozvat?</h2><p class="muted">Po chvíli se může jen zeptat, jestli chceš malé postrčení. <b>Nic neodhalí bez tvého souhlasu.</b></p><div class="support-choice-grid onboard-support-grid" aria-label="Čas nabídky Pomocníka">${supportChoicesHtml('onboard')}</div><div id="onboardSupportOutcome" class="support-outcome" aria-live="polite">Vyber, jak dlouho chceš nejdřív přemýšlet sám.</div></div>`}
];
...
async function openPlayedLevels(diff){
 ...
 const actual=summary?.actual??levels.filter(r=>!r.transferred).length,transferred=summary?.transferred??levels.filter(r=>r.transferred).length,total=summary?.total??sortedFreeBank(diff).length;$('#playedLevelsMeta').textContent=levels.length?`${actual} skutečně hraných v Gen2${transferred?` · ${transferred} převedených z původní banky`:''} · postup ${levels.length}/${total}`:'Zatím tu nic není. Nejdřív něco propleť.';
...
function renderFreeTeamBoard(data,error,myId){
 ...
 if(data?.anonymous)return '<div class="leaderboard-empty"><strong>Přihlas se a porovnej se s týmem.</strong><small>Žebříček srovnává přesně tuhle úroveň.</small></div>';
...
function bind(){
 ...
 $('#backFromGame').onclick=goBackFromGame;$('#resetBtn').onclick=resetGame;$('#hintBtn').onclick=openHintModal;$('#winPrimaryBtn').onclick=closeWinAndContinue;$('#winReplayBtn').onclick=replayDailyFromWin;$('#winMenuBtn').onclick=closeWinToMenu;$('#winShareBtn').onclick=shareDaily;
