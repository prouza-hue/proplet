...
function savedProgressFor(puzzle,mode,dailyDate){
 ...
 return {...r,found,moves:Math.max(0,Number(r.moves)||0),hints:Math.max(0,Number(r.hints)||0),wrongAttempts:Math.max(0,Number(r.wrongAttempts)||0),maxHintLevel:Math.max(0,Number(r.maxHintLevel)||0),resetCount:Math.max(0,Number(r.resetCount)||0),firstHintAtMs:r.firstHintAtMs==null?null:Math.max(0,Number(r.firstHintAtMs)||0),firstCorrectAtMs:r.firstCorrectAtMs==null?null:Math.max(0,Number(r.firstCorrectAtMs)||0),lastCorrectAtMs:r.lastCorrectAtMs==null?null:Math.max(0,Number(r.lastCorrectAtMs)||0),elapsedMs:Math.max(0,Number(r.elapsedMs)||0)};
}
...
function applyScreen(screen){
 ...
 if(prev==='game'&&screen!=='game'){if(currentGame?.mode!=='rescue'){saveGameProgress();sendAttemptCheckpoint(currentGame,{keepalive:true})}stopTimer()}
...
async function startAttemptTelemetry(g,resuming=false){const p=getProfile();if(!p?.token||!g||g.mode==='rescue')return;try{await api('/api/attempt/start',{method:'POST',body:JSON.stringify({attempt_id:g.attemptId,puzzle_id:g.puzzle.id,challenge_key:challengeKey(g.mode,g.puzzle,g.dailyDate),mode:g.mode,difficulty:g.puzzle.difficulty,resuming:!!resuming})})}catch{}}
function attemptCheckpointPayload(g=currentGame){if(!g||g.mode==='rescue'||g.finished)return null;return {attempt_id:g.attemptId,elapsed_ms:Math.round(gameElapsed(g)),moves:g.moves||0,found_words:g.found?.length||0,hints_used:g.hints||0,wrong_attempts:g.wrongAttempts||0,max_hint_level:g.maxHintLevel||0,reset_count:g.resetCount||0,first_hint_at_ms:g.firstHintAtMs??null,first_correct_at_ms:g.firstCorrectAtMs??null,last_correct_at_ms:g.lastCorrectAtMs??null}}
async function sendAttemptCheckpoint(g=currentGame,{keepalive=false}={}){const p=getProfile(),body=attemptCheckpointPayload(g);if(!p?.token||!body)return;try{await api('/api/attempt/checkpoint',{method:'POST',body:JSON.stringify(body),keepalive})}catch{}}
function startGame(puzzle,mode,dailyDate,options={}){
 ...
 $('#timer').textContent=mode==='rescue'?fmtCountdown(remaining):fmtTime(baseElapsedMs);message(restored?(mode==='daily'?'Pokračuješ. U Denní výzvy čas běžel i mimo herní obrazovku.':'Pokračuješ přesně tam, kde jsi skončil.'):'Najdi slova. A pamatuj: nestačí slovo, musí sedět i cesta.');nav('game');renderGameBoard();renderGameHUD();startTimer();if(mode!=='rescue')saveGameProgress();startAttemptTelemetry(currentGame,!!restored);
}
...
function submitPath(){
 ...
 if(ai>=0){const colorIndex=g.found.length%COLORS.length,path=[...g.path],nowMs=Math.round(gameElapsed(g));g.found.push({answerIndex:ai,word,colorIndex,path});path.forEach(i=>g.used.set(i,colorIndex));g.lastFound=path;if(g.firstCorrectAtMs==null)g.firstCorrectAtMs=nowMs;g.lastCorrectAtMs=nowMs;message(`✓ ${word}`,'good');fx('correct')}
 ...
 g.path=[];renderGameBoard();renderGameHUD();$('#currentWord').textContent='—';if(g.mode!=='rescue'){saveGameProgress();if(ai>=0)sendAttemptCheckpoint(g)}if(g.found.length===g.puzzle.answers.length){if(g.mode==='rescue')finishRescue(true);else finishGame();}
}
function resetGame(){const g=currentGame;if(g.mode==='rescue')return;const usedHints=g.hints||0,elapsed=gameElapsed(g);g.resetCount=(g.resetCount||0)+1;g.found=[];g.used=new Map();g.path=[];g.baseElapsedMs=elapsed;g.start=performance.now();g.elapsedMs=elapsed;g.lastFound=[];g.hints=usedHints;g.cleanSolve=usedHints===0;message(usedHints?'Plocha čistá. Čas, tahy i nápovědy běží dál. Čisté řešení už zpátky není.':'Plocha čistá. Čas i tahy běží dál — pořád je to stejný pokus.');renderGameBoard();renderGameHUD();saveGameProgress();sendAttemptCheckpoint(g)}
...
function bind(){
 ...
 document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'){saveGameProgress();sendAttemptCheckpoint(currentGame,{keepalive:true})}else if(getQueue().length)syncQueue({announce:false})});window.addEventListener('pagehide',()=>{saveGameProgress();sendAttemptCheckpoint(currentGame,{keepalive:true})});
}


function qaPct(v){return v==null?'—':`${Math.round(Number(v)*100)} %`}
function qaIndex(v){if(v==null)return '—';const n=Number(v);return `${n>0?'+':''}${n.toFixed(2)}`}
function qaPuzzleLabel(r){if(r.mode==='daily')return `☀️ ${r.puzzleId}`;const d=DIFF[r.difficulty]||{icon:'🧩',label:r.difficulty};return `${d.icon} ${d.label}${r.level?` ${r.level}`:''}`}
function qaStatusLabel(s){return ({too_hard:'🔴 příliš těžká',too_easy:'🟠 příliš lehká',word_review:'🟣 slova ke kontrole',watch:'🟡 sledovat',ok:'🟢 v normě',waiting:'⚪ čeká na data'})[s]||s||'—'}
function qaRowHtml(r){return `<div class="qa-row"><div class="qa-main"><strong>${esc(qaPuzzleLabel(r))}</strong><small>${qaStatusLabel(r.status)} · ${esc(r.confidence?.label||'bez dat')}</small></div><div class="qa-metrics"><span><b>${qaIndex(r.difficultyIndex)}</b><small>index</small></span><span><b>${r.starts||0}</b><small>první pokusy</small></span><span><b>${qaPct(r.completionRate)}</b><small>dokončeno</small></span><span><b>${r.medianMs?fmtTime(r.medianMs):'—'}</b><small>medián</small></span><span><b>${r.avgHints==null?'—':Number(r.avgHints).toFixed(1)}</b><small>nápovědy</small></span><span><b>${qaPct(r.cleanRate)}</b><small>čistě</small></span><span><b>${r.difficultyRating==null?'—':`${Number(r.difficultyRating)>0?'+':''}${Number(r.difficultyRating).toFixed(2)}`}</b><small>rating</small></span></div><p>${esc(r.recommendation||'')}</p></div>`}
function qualitySummaryText(data){const s=data.summary||{},lines=[`Proplet QA v${data.version||'?'} · ${new Date(data.generatedAt||Date.now()).toLocaleString('cs-CZ')}`,`První pokusy: ${s.firstExposures||0} · puzzle s daty: ${s.puzzlesMeasured||0} · alerty: ${s.alerts||0} · watch: ${s.watch||0}`,`Ratingy obtížnosti: ${s.difficultyRatings||0} · reporty slov: ${s.wordReports||0}`,''];for(const r of (data.alerts||[]).slice(0,20))lines.push(`${qaPuzzleLabel(r)} | ${qaStatusLabel(r.status)} | DI ${qaIndex(r.difficultyIndex)} | n=${r.starts} | dokončeno ${qaPct(r.completionRate)} | čas ${r.medianMs?fmtTime(r.medianMs):'—'} | hint ${r.avgHints==null?'—':Number(r.avgHints).toFixed(1)} | clean ${qaPct(r.cleanRate)} | rating ${r.difficultyRating==null?'—':r.difficultyRating}`);if(!(data.alerts||[]).length)lines.push('Žádný spolehlivý outlier zatím nenalezen.');return lines.join('\n')}
async function openQualityDashboardFromQuery(){
 if(!new URLSearchParams(location.search).has('qa'))return;
 const overlay=document.createElement('div');overlay.className='qa-overlay';overlay.innerHTML=`<section class="qa-panel"><div class="qa-head"><div><small>INTERNÍ · QUALITY ANALYTICS V2</small><h2>🧪 Kvalita Propletu</h2></div><button class="icon-btn" id="qaCloseBtn" aria-label="Zavřít">✕</button></div><div id="qaBody" class="qa-body"><div class="gate card">Načítám telemetry…</div></div></section>`;document.body.appendChild(overlay);
 const close=()=>{overlay.remove();const u=new URL(location.href);u.searchParams.delete('qa');history.replaceState(history.state,'',u.pathname+u.search+u.hash)};overlay.querySelector('#qaCloseBtn').onclick=close;
 const body=overlay.querySelector('#qaBody');
 if(!getProfile()?.token){body.innerHTML='<div class="gate card"><strong>Nejdřív se přihlas.</strong><p>QA report je dostupný pouze přihlášenému playtesterovi. Po přihlášení znovu otevři adresu s <code>?qa=1</code>.</p></div>';return}
 try{
  const data=await api('/api/quality-report'),s=data.summary||{},alerts=data.alerts||[],watch=data.watch||[],changes=data.notableChanges||[];
  body.innerHTML=`<div class="qa-summary"><div><b>${s.firstExposures||0}</b><small>prvních pokusů</small></div><div><b>${s.puzzlesMeasured||0}</b><small>puzzle s daty</small></div><div><b>${s.alerts||0}</b><small>spolehlivé alerty</small></div><div><b>${s.watch||0}</b><small>ke sledování</small></div></div><div class="qa-actions"><button class="secondary" id="qaCopyBtn">📋 Kopírovat shrnutí</button><small>${data.previousSnapshotDate?`Srovnání s ${esc(String(data.previousSnapshotDate))}`:'První týdenní snapshot se uloží automaticky v pondělí.'}</small></div><section class="qa-section"><h3>🔎 Vyžaduje pozornost</h3>${alerts.length?alerts.slice(0,30).map(qaRowHtml).join(''):'<div class="qa-empty">Zatím žádný spolehlivý outlier. To je dobrá zpráva — nebo jen potřebujeme více hráčů. 🙂</div>'}</section><section class="qa-section"><h3>🟡 Předběžné signály</h3>${watch.length?watch.slice(0,20).map(qaRowHtml).join(''):'<div class="qa-empty">Nic výrazného.</div>'}</section>${changes.length?`<section class="qa-section"><h3>↕️ Největší změny od snapshotu</h3>${changes.slice(0,12).map(c=>`<div class="qa-change"><strong>${esc(c.puzzleId)}</strong><span>${c.delta>0?'těžší':'lehčí'} o ${Math.abs(c.delta).toFixed(2)}</span></div>`).join('')}</section>`:''}<section class="qa-method"><strong>Jak číst Difficulty Index</strong><p>0 = typická úloha své skupiny. Kladná hodnota = chová se těžší, záporná = lehčí. Hlavní model používá jen první setkání hráče s puzzle; replaye jsou diagnostika. Automatický zásah do obsahu se nedělá nikdy.</p></section>`;
  body.querySelector('#qaCopyBtn').onclick=async()=>{try{await navigator.clipboard.writeText(qualitySummaryText(data));showToast('QA shrnutí je ve schránce ✓')}catch{showToast('Kopírování se nepovedlo')}};
 }catch(e){body.innerHTML=`<div class="gate card"><strong>QA report se nepodařilo načíst.</strong><p>${esc(e.message)}</p><small>Po v3.13 zkontroluj hlavně SQL migraci a <code>qualityAnalyticsV2: true</code> v /api/health.</small></div>`}
}
...
async function boot(){
 ...
 registerServiceWorker();setTimeout(updatePushUI,700);setTimeout(openQualityDashboardFromQuery,900);
