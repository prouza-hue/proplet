...
function renderDailyGlobalLeaderboardBox(container,data){
 const total=Number(data?.total||0),rank=Number(data?.myRank||0),rows=data?.rows||[];container.classList.add('daily-global-board');container.classList.remove('hidden');
 ...
 container.innerHTML=`<div class="daily-world-head"><strong>🌍 Dnešní globální pořadí</strong><span>${countCz(total,'hráč','hráči','hráčů')}</span></div><div class="daily-world-summary"><div><strong>${rank}.</strong><span>místo</span></div><p>${topLine}<small>Clean → méně nápověd → čas → tahy</small></p></div><div class="daily-world-neighbours">${rows.map(r=>`<div class="mini-leader-row ${r.isMine?'me':''}"><b>${r.rank===1?'🥇':r.rank===2?'🥈':r.rank===3?'🥉':r.rank+'.'}</b><span><strong>${r.isMine?'Ty':'Soupeř'}</strong><small>${r.cleanSolve?'✨ Čistě':`💡 ${r.hintsUsed||0}×`} · ${countCz(r.moves,'tah','tahy','tahů')}</small></span><em>${fmtTime(r.elapsedMs)}</em></div>`).join('')}</div><small class="daily-world-privacy">Jména ostatních hráčů zůstávají soukromá.</small>`;
}
