...
def test_starter_exists_and_health():
    ...
    with patch.object(server,'supabase_ready',return_value=False):
        h=server.health()
    assert h['version']=='3.21.3' and h['gameFeelSprint']=='3.21' and h['starterPuzzle'] is True and h['starterXp']==10
...
def test_starter_product_events_are_allowed():
    ...
    assert all(row['app_version']=='3.21.3' for _,row in inserted)
