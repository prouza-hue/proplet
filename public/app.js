...
function renderFree(){
 ...
 $('#difficultyCards').innerHTML=Object.entries(DIFF).map(([key,d])=>{
  const {total,done,actual,transferred,pct,resume,nextUnsolved,slots}=freeProgress(key),nextLevel=Number((resume||nextUnsolved)?.meta?.level)||null,progressLabel=resume?`ROZEHRÁNO${nextLevel?` · ÚROVEŇ ${nextLevel}`:''}`:(done===total?`${done}/${total} HOTOVO`:`ÚROVEŇ ${nextLevel||1} Z ${total}`),historicalXp=nextLevel&&slots.prior.has(nextLevel)&&!slots.actual.has(nextLevel),xpLabel=historicalXp?'Nová deska · XP už započítané':`+${d.xp} XP za novou úroveň`;
