...
async function openPlayedLevels(diff){
 const d=DIFF[diff],modal=$('#playedLevelsModal');$('#playedLevelsTitle').innerHTML=`${difficultyIconMarkup(diff,'played-levels-title-icon')}<span>${esc(d.label)} · tvoje úrovně</span>`;$('#playedLevelsMeta').textContent='Načítám odehrané a převedené úrovně…';$('#playedLevelsList').innerHTML='';modal.classList.remove('hidden');const p=getProfile();let levels=[],legacyLevels=[],summary=null;
