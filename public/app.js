...
const MOZKOMOR_REFRESH_PREVIEW=MOZKOMOR_QA_PARAM==='refresh';
...
function gameplayStateScope(){const scope=playerScope();return MOZKOMOR_REFRESH_PREVIEW?`${scope}:mozkomor-human-refresh-v40131`:scope}
...
function applyScreen(screen){
 ...
 if(prev==='game'&&screen!=='game'){releaseGameWakeLock();pauseGameClock('menu');if(currentGame?.mode!=='rescue'){saveGameProgress();sendAttemptCheckpoint('leave')}stopTimer()}
...
async function refreshRescueStatus(){
 ...
 if(puzzleDB){renderDaily();maybeOfferRescue()}
 return rescueStatus;
}
...
function mozkomorUnlockState(){
 ...
 const unlocked=MOZKOMOR_REFRESH_PREVIEW||MOZKOMOR_PREVIEW_UNLOCK||remembered||remote.mozkomorUnlocked===true||done>=required;
 if(unlocked&&!MOZKOMOR_PREVIEW_UNLOCK&&!MOZKOMOR_REFRESH_PREVIEW)try{localStorage.setItem(key,'1')}catch{}
...
function renderFree(){
 ...
 $('#difficultyCards').innerHTML=Object.entries(DIFF).map(([key,d])=>{
  ...
  const {total,done,actual,transferred,pct,resume,nextUnsolved}=freeProgress(key),nextLevel=Number((resume||nextUnsolved)?.meta?.level)||null,isRefresh=key==='mozkomor'&&MOZKOMOR_REFRESH_PREVIEW,progressLabel=isRefresh?'🧪 PLAYTEST · REFRESH 10':resume?`ROZEHRÁNO${nextLevel?` · ÚROVEŇ ${nextLevel}`:''}`:(done===total?`${done}/${total} HOTOVO`:`ÚROVEŇ ${nextLevel||1} Z ${total}`),xpLabel=isRefresh?'Bez XP · kalibrační sada':`+${d.xp} XP za novou úroveň`,cardPlayAttr=key==='mozkomor'?` data-play-free="mozkomor" role="button" tabindex="0" aria-label="Hrát Mozkomor"`:'';
  return `<article class="difficulty-card card ${key==='mozkomor'?'mozkomor-unlocked':''} ${isRefresh?'mozkomor-refresh':''}" data-diff="${key}"${cardPlayAttr}><div class="difficulty-copy"><div class="difficulty-title"><span class="difficulty-left-icon">${difficultyIconMarkup(key,'difficulty-icon-img')}</span><div><span class="eyebrow">${key==='mozkomor'&&!done&&!isRefresh?'🌑 ODEMČENO · ENDGAME':progressLabel}</span><div class="difficulty-heading-line"><h2>${d.label}</h2></div></div></div><p class="muted">${d.desc}</p><span class="xp-chip">${xpLabel}</span><div class="progress-line"><span style="width:${pct}%"></span></div><div class="difficulty-actions"><button class="secondary-btn play-next-btn" data-play-free="${key}">${resume?'Pokračovat':(done===total?'Hrát znovu':'Hrát další úroveň')}</button><button class="text-btn played-levels-btn" data-played-levels="${key}" ${done||transferred?'':'disabled'}>▦ Postup a úrovně${done?` · ${done} splněných`:''}</button></div></div><div class="difficulty-progress" data-play-free="${key}" role="button" tabindex="0" aria-label="${resume?'Pokračovat v rozehrané':'Hrát'} ${d.label}" style="--progress:${pct}%"><div><strong>${done}</strong><small>/${total}</small></div><span>›</span></div></article>`
 }).join('');
...
async function startAttemptTelemetry(g){if(MOZKOMOR_REFRESH_PREVIEW||CONTENT_PREVIEW_DATE||GEN4_CANDIDATE_PREVIEW||!g||g.mode==='rescue'||g.mode==='starter')return;try{await api('/api/attempt/start',{method:'POST',body:JSON.stringify({attempt_id:g.attemptId,puzzle_id:g.puzzle.id,challenge_key:challengeKey(g.mode,g.puzzle,g.dailyDate),mode:g.mode,difficulty:g.puzzle.difficulty})})}catch{}}
async function sendAttemptCheckpoint(eventType){
 const g=currentGame;if(MOZKOMOR_REFRESH_PREVIEW||CONTENT_PREVIEW_DATE||GEN4_CANDIDATE_PREVIEW||!g||g.mode==='rescue'||g.mode==='starter'||g.finished)return;
...
function startGame(puzzle,mode,dailyDate,options={}){
 ...
 $('#timer').textContent=mode==='rescue'?fmtCountdown(remaining):fmtTime(baseElapsedMs);message(restored?'Pokračuješ přesně tam, kde jsi skončil.':mode==='starter'?'':'Propleť všechna políčka. Slova můžou zatáčet.');$('#starterCoach')?.classList.toggle('hidden',mode!=='starter');nav('game');renderGameBoard();renderGameHUD();updateGameFeel();syncGameWakeLock();if(mode==='starter'){trackProductEvent('starter_started');updateStarterGuidance()}startTimer();if(mode!=='rescue'&&mode!=='starter')saveGameProgress();startAttemptTelemetry(currentGame).then(()=>{if(restored)sendAttemptCheckpoint('resume')});
}
...
function renderGameHUD(){
 ...
 const clean=$('#cleanStatus');clean.textContent=g.mode==='rescue'?'':g.mode==='starter'?'🎓 Trénink':(g.hints?'💡 S nápovědou':'✨ Čistě');clean.classList.toggle('lost',g.mode!=='starter'&&!!g.hints);$('#hintBtn').textContent=g.mode==='starter'?'💡 Nápověda':g.hints?`💡 ${g.hints}×`:'💡 Nápověda';renderMagnifierControls();updateGameFeel();
}
...
async function finishAttemptTelemetry(rec){
 if(MOZKOMOR_REFRESH_PREVIEW||CONTENT_PREVIEW_DATE||GEN4_CANDIDATE_PREVIEW||!rec?.attemptId||rec.mode==='rescue'||rec.mode==='starter')return;
...
async function finishStarterGame(g){
 g.finished=true;g.justCompleted=true;g.elapsedMs=gameElapsed(g);stopTimer();releaseGameWakeLock();g.starterGuidePath=[];hideStarterHintNudge();renderGameBoard();renderGameHUD();updateGameFeel();
...
function shouldOfferAccountNudge(){
 if(MOZKOMOR_REFRESH_PREVIEW||getProfile()?.token||currentGame?.mode==='rescue'||currentGame?.mode==='starter'||currentGame?.postStarterWarmup||currentGame?.justCompleted!==true)return 0;
...
function queueResult(rec){
 if(GEN4_CANDIDATE_PREVIEW)return;
...
function shouldOfferInstallNudge(source='daily'){
 if(MOZKOMOR_REFRESH_PREVIEW)return false;
...
async function shouldOfferPushNudge(){
 const g=currentGame;if(MOZKOMOR_REFRESH_PREVIEW||!['daily','free'].includes(g?.mode)||g?.justCompleted!==true||!pushNudgeDue())return false;if(!('Notification' in window)||!('PushManager' in window)||Notification.permission==='denied')return false;
...
async function loadPuzzleDatabase(){
 ...
 if(MOZKOMOR_REFRESH_PREVIEW){
  const [baseResponse,playtestResponse]=await Promise.all([fetch('/puzzles.json',{cache:'no-store'}),fetch('/mozkomor-refresh-playtest.json',{cache:'no-store'})]);
  if(!baseResponse.ok||!playtestResponse.ok)throw new Error('mozkomor-refresh-db');
  const data=await baseResponse.json(),playtest=await playtestResponse.json();
  if(!compatiblePuzzleDatabase(data)||playtest?.kind!=='mozkomor-human-refresh-playtest'||(playtest?.puzzles||[]).length!==10)throw new Error('mozkomor-refresh-db-version');
  data.free={...(data.free||{}),mozkomor:playtest.puzzles};data.mozkomorRefreshPlaytest={kind:playtest.kind,count:10,sourceBank:playtest.sourceBank};return data;
 }
