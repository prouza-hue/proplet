...
function pointsFor(mode,difficulty,puzzle=null){
 if(mode==='tajenka')return 0;if(mode==='daily')return 100;if(mode==='starter')return Number(puzzle?.meta?.rewardXp)||10;if(mode!=='free')return DIFF[difficulty].xp;
...
function startTajenka(){if(!TAJENKA_PREVIEW||!tajenkaPuzzle){showToast('Testovací Tajenka není na této verzi dostupná.');return}trackProductEvent('tajenka_started');startGame(tajenkaPuzzle,'tajenka',null)}

function newAttemptId(){try{return crypto.randomUUID()}catch{return `a-${Date.now()}-${Math.random().toString(36).slice(2,10)}`}}
async function startAttemptTelemetry(g){if(CONTENT_PREVIEW_DATE||GEN4_CANDIDATE_PREVIEW||!g||g.mode==='rescue'||g.mode==='starter'||g.mode==='tajenka')return;try{await api('/api/attempt/start',{method:'POST',body:JSON.stringify({attempt_id:g.attemptId,puzzle_id:g.puzzle.id,challenge_key:challengeKey(g.mode,g.puzzle,g.dailyDate),mode:g.mode,difficulty:g.puzzle.difficulty})})}catch{}}
async function sendAttemptCheckpoint(eventType){
 const g=currentGame;if(CONTENT_PREVIEW_DATE||GEN4_CANDIDATE_PREVIEW||!g||g.mode==='rescue'||g.mode==='starter'||g.mode==='tajenka'||g.finished)return;
...
function startGame(puzzle,mode,dailyDate,options={}){
 ...
 $('#timer').textContent=mode==='rescue'?fmtCountdown(remaining):fmtTime(baseElapsedMs);message(restored?'Pokračuješ přesně tam, kde jsi skončil.':mode==='starter'?'':mode==='tajenka'?'Najdi slova a odhal skrytou frázi.':'Propleť všechna políčka. Slova můžou zatáčet.');$('#starterCoach')?.classList.toggle('hidden',mode!=='starter');nav('game');renderGameBoard();renderGameHUD();updateGameFeel();syncGameWakeLock();if(mode==='starter'){trackProductEvent('starter_started');updateStarterGuidance()}if(mode==='tajenka')renderTajenkaPhrase(currentGame);startTimer();if(mode!=='rescue'&&mode!=='starter')saveGameProgress();startAttemptTelemetry(currentGame).then(()=>{if(restored)sendAttemptCheckpoint('resume')});
}
...
function renderTajenkaPhrase(g=currentGame){
 ...
 root.classList.remove('hidden');root.innerHTML=`<div class="tajenka-phrase-head"><span class="stat-label">TAJENKA</span><strong>${g.found.length}/${words.length}</strong></div><div class="tajenka-slots">${words.map((answer,i)=>{const revealed=found.has(answerIndexForTajenka(g.puzzle,i));return `<span class="tajenka-slot ${revealed?'revealed':'pending'}" ${revealed?'':'aria-label="Skryté slovo"'}>${revealed?esc(answer.word):'·'.repeat(answer.word.length)}</span>`}).join('<b class="tajenka-space" aria-hidden="true">·</b>')}</div><small>${complete?'Tajenka je kompletní.':'Hledej slova v mřížce — každé odhalí část fráze.'}</small>`;
}
...
function submitPath(){
 ...
 let wrong=false;
 if(ai>=0){const colorIndex=g.found.length%COLORS.length;g.found.push({answerIndex:ai,word,colorIndex,path});path.forEach(i=>g.used.set(i,colorIndex));g.lastFound=path;g.lastProgressAt=performance.now();if(g.mode==='tajenka')trackProductEvent('tajenka_word_found');else sendAttemptCheckpoint('correct');message(`✓ ${word}`,'good');fx('correct')}
...
function openHintModal(fromHelper=false){if(!currentGame||currentGame.mode==='rescue'||currentGame.finished)return;const g=currentGame;if(g.undoSnapshot)hideGameUndo();if(g.mode==='starter')hideStarterHintNudge();if(!fromHelper)g.nextHintSource='manual';const starter=g.mode==='starter',tajenka=g.mode==='tajenka';$('#hintModal').classList.toggle('starter-hint',starter);$('#hintEyebrow').textContent=starter?'TRÉNINKOVÁ NÁPOVĚDA':tajenka?'TAJENKA · NÁPOVĚDA':'CHYTRÁ NÁPOVĚDA';$('#hintTitle').textContent=starter?'Zkus malé postrčení':tajenka?'Chceš malé postrčení?':'Kolik pomoci chceš?';$('#hintCopy').textContent=starter?'V běžné hře nápověda zruší ✨ Čistě. Tady si ji můžeš bezpečně vyzkoušet.':tajenka?'Nápověda je jen pro testovací desku a neovlivní žádné XP ani pořadí.':'Jakákoli nápověda zruší ✨ čisté řešení této úrovně. Nápovědy zatím nejsou omezené.';$('#hintModal').classList.remove('hidden')
}
...
function trackTajenkaAbandon(g=currentGame){if(!TAJENKA_PREVIEW||!g||g.mode!=='tajenka'||g.finished||g.tajenkaAbandonTracked)return;g.tajenkaAbandonTracked=true;trackProductEvent('tajenka_abandoned')}
function trackTajenkaView(){if(!TAJENKA_PREVIEW)return;try{if(sessionStorage.getItem(TAJENKA_VIEW_KEY)==='1')return;sessionStorage.setItem(TAJENKA_VIEW_KEY,'1')}catch{}trackProductEvent('tajenka_viewed')}
...
async function loadTajenkaFixture(){
 if(!TAJENKA_PREVIEW)return null;
 try{const response=await fetch('/tajenka-test.json',{cache:'no-store'});if(!response.ok)throw new Error('tajenka-fixture');const data=await response.json();if(!tajenkaFixtureValid(data))throw new Error('tajenka-fixture-invalid');tajenkaPuzzle=data;return data}catch(error){console.warn('Tajenka preview fixture unavailable',error);tajenkaPuzzle=null;return null}
}
