(function(root,factory){
  'use strict';
  if(root?.PropletEngagementNudges){
    if(typeof module!=='undefined'&&module.exports)module.exports=root.PropletEngagementNudges;
    return;
  }
  const api=factory(root);
  if(typeof module!=='undefined'&&module.exports)module.exports=api;
  if(root)root.PropletEngagementNudges=api;
  ...
  async function runPostWin(action,deps=dependencies){
    if(await deps.maybeOfferFirstWinReturnNudge(action))return true;
    if(deps.maybeOfferAccountNudge(action))return true;
    if(await deps.maybeOfferPushNudge(action))return true;
    if(deps.maybeOfferInstallNudge(action,'daily'))return true;
    deps.hideWin();
    deps.performPostWinAction(action);
    return false;
  }

  function bindInstallLifecycle(deps){
    if(installLifecycleBound||!root?.addEventListener)return;
    installLifecycleBound=true;
    root.addEventListener('beforeinstallprompt',event=>deps.onBeforeInstallPrompt(event));
    root.addEventListener('appinstalled',()=>deps.onAppInstalled());
  }

  function installDifficulty(){
  ...
  function install(deps={}){
    ...
    bindInstallLifecycle(dependencies);
    installDifficulty();
    return api;
  }

  const api={install,runPostWin};
  return api;
});
