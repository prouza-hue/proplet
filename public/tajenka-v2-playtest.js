(()=>{
'use strict';
const DATA_URL='https://raw.githubusercontent.com/prouza-hue/proplet/ux/tiskarska-dilna/data/tajenka_v2_board_candidates.json';
const STORE='proplet-tajenka-v2-playability-41-v1';
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
const categoryLabels={observation:'Pozorování',dry_humor:'Suchý humor',fact:'Fakt',language:'Jazyk',proverb:'Rčení',quote:'Citát'};
...
function debug(){
  const m=puzzle.meta||{};const vals=[['Písmena',m.targetLetters],['Decoye',m.decoyCells],['Otvory',m.holes],['Kontakty',m.crossWordEdges],['Páry slov',m.crossWordPairs],['Score',m.qualityScore],['Alt. full path',m.alternativeFullPaths],['Anchory',puzzle.answers.length]];
...
function setRating(r){$$('.rate').forEach(b=>b.classList.toggle('active',b.dataset.rating===r));upsert(puzzle.id,{candidateNo:puzzle.candidateNo,category:puzzle.category,rating:r,note:$('#note').value.trim(),reviewedAt:new Date().toISOString()});$('#savedState').textContent='Uloženo.';setTimeout(()=>$('#savedState').textContent='Hodnocení se ukládá automaticky.',900)}
function saveNote(){const active=$('.rate.active')?.dataset.rating;upsert(puzzle.id,{candidateNo:puzzle.candidateNo,category:puzzle.category,rating:active||getRow(puzzle.id)?.rating||null,note:$('#note').value.trim()})}
function renderSession(){
  if(!boards.length)return;const s=loadStore(),rows=boards.map(b=>({b,r:s[b.id]})).filter(x=>x.r);const reviewed=rows.filter(x=>x.r.rating);const completed=rows.filter(x=>x.r.completed);$('#overall').textContent=`${reviewed.length} / ${boards.length}`;$('#completionPill').textContent=`${completed.length} dokončeno`;const counts={ok:0,easy:0,hard:0,bad:0};reviewed.forEach(x=>counts[x.r.rating]=(counts[x.r.rating]||0)+1);$('#summaryChips').innerHTML=Object.keys(counts).map(k=>`<span class="summary-chip">${ratingLabels[k]}: ${counts[k]}</span>`).join('');$('#results').innerHTML=rows.length?rows.map(({b,r})=>`<div class="row"><strong>#${b.candidateNo}</strong><span>${esc((b.displayText||'').slice(0,62))}</span><time>${r.elapsedMs?fmt(r.elapsedMs):'—'}</time><span class="rating-badge rating-${esc(r.rating||'')}">${esc(r.rating?ratingLabels[r.rating]:'bez hodnocení')}</span></div>`).join(''):'<span style="color:var(--muted);font-size:12px">Zatím nic.</span>';populateSelect()
}
function populateSelect(){const s=loadStore(),current=String(pos);$('#boardSelect').innerHTML=boards.map((b,i)=>`<option value="${i}">#${b.candidateNo} · ${esc(categoryLabels[b.category]||b.category)}${s[b.id]?.rating?' ✓':''}</option>`).join('');$('#boardSelect').value=current}
async function copyReport(){const s=loadStore();const rows=boards.filter(b=>s[b.id]?.rating||s[b.id]?.completed).map(b=>({b,r:s[b.id]}));if(!rows.length)return;const text=['Proplet · Tajenka v2 · playability 41',...rows.map(({b,r})=>`#${b.candidateNo} ${r.rating?ratingLabels[r.rating]:'bez hodnocení'} · ${r.elapsedMs?fmt(r.elapsedMs):'—'} · ${r.moves??'—'} tahů · ${r.wrong??'—'} chyb${r.note?` · ${r.note}`:''}`)].join('\n');try{await navigator.clipboard.writeText(text);$('#copy').textContent='Zkopírováno ✓';setTimeout(()=>$('#copy').textContent='Kopírovat report',1200)}catch{prompt('Zkopíruj report:',text)}}
function bind(){const board=$('#board');board.addEventListener('pointerdown',e=>{const c=e.target.closest('.cell[data-cell]');if(!c)return;e.preventDefault();selectStart(Number(c.dataset.cell))});board.addEventListener('pointermove',e=>{if(!dragging)return;e.preventDefault();const el=document.elementFromPoint(e.clientX,e.clientY),c=el?.closest?.('.cell[data-cell]');if(c)selectMove(Number(c.dataset.cell))});window.addEventListener('pointerup',submit,{passive:true});window.addEventListener('pointercancel',()=>{dragging=false;selected=[];paint();updateCurrent()},{passive:true});$('#startOverlay').onclick=start;$('#restart').onclick=()=>loadBoard(pos);$('#skip').onclick=next;$('#reveal').onclick=revealSolution;$('#nextUnrated').onclick=nextUnrated;$('#boardSelect').onchange=()=>loadBoard(Number($('#boardSelect').value)||0);$$('.rate').forEach(b=>b.onclick=()=>setRating(b.dataset.rating));$('#note').addEventListener('change',saveNote);$('#saveNext').onclick=()=>{saveNote();nextUnrated()};$('#copy').onclick=copyReport;$('#clear').onclick=()=>{if(confirm('Opravdu smazat celý lokální Tajenka v2 playtest?')){localStorage.removeItem(STORE);renderSession();loadBoard(pos)}}}
async function boot(){try{const r=await fetch(DATA_URL,{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);db=await r.json();boards=db.boards||[];if(boards.length!==41)throw new Error(`čekám 41 desek, mám ${boards.length}`)}catch(e){$('#message').textContent=`Data se nepodařilo načíst: ${e.message}`;$('#startOverlay').disabled=true;return}populateSelect();bind();const q=Number(new URLSearchParams(location.search).get('board'));const qp=Number.isFinite(q)?boards.findIndex(b=>b.candidateNo===q):-1;const s=loadStore();let initial=qp>=0?qp:boards.findIndex(b=>!s[b.id]?.rating);if(initial<0)initial=0;loadBoard(initial);renderSession()}
boot();
})();
