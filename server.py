...
def db_atomic_result_rpc(function: str, body: Optional[dict] = None):
    return backend_db.db_rpc(
        function,
        body,
        supabase_url=SUPABASE_URL,
        ...
        http_client=DB_HTTP_CLIENT,
        httpx_client_factory=httpx.Client,
        error_mapper=result_domain.map_atomic_rpc_error,
    )
...
def atomic_result_public_response(player_id: str, receipt: dict) -> dict:
    """Refresh read-only stats after the atomic commit or durable replay."""
    try:
        stats = player_stats(player_id)
        stats_warning = None
    except Exception:
        logger.exception("Atomic result saved, but stats refresh failed")
        stats = None
        stats_warning = "Statistiky se nepodařilo obnovit. Výsledek je ale bezpečně uložený."
    return result_domain.public_response(receipt, stats, stats_warning)
...
def result(payload: ResultCreate, request: Request, authorization: Optional[str] = Header(default=None)):
    ...
    atomic_identity = None
    if ATOMIC_RESULT_V1_ENABLED:
        atomic_identity = result_domain.result_identity(player["id"], payload)
        try:
            replay_receipt = result_domain.existing_receipt(atomic_identity, db_select)
        except HTTPException as exc:
            if exc.status_code in (409, 503):
                raise
            raise HTTPException(503, "Atomické ukládání výsledku není připravené") from exc
        if replay_receipt is not None:
            logger.info("atomic_result_v1 status=replay")
            return atomic_result_public_response(player["id"], replay_receipt)
    if not puzzle_exists(payload.puzzle_id, payload.mode, payload.difficulty):
        raise HTTPException(400, "Neznámá úloha")
    ...
    if payload.mode == "tajenka":
        ...
        daily_info = puzzle_info(payload.puzzle_id) or {}
        content_generation = daily_info.get("generation")
    else:
        ...
        content_generation = int(info.get("generation") or 1)
        free_level = int(info["level"])
        base_points = domain_content.xp_for("free", payload.difficulty, POINTS)
        if ATOMIC_RESULT_V1_ENABLED:
            active_generation = int(load_puzzles().get("freeGeneration") or 1)
            is_current = content_generation == active_generation and info.get("legacy") is not True
            if active_generation >= 4 and is_current:
                points = int(base_points)
            else:
                historical_reward = free_slot_already_rewarded(
                    player["id"], payload.difficulty, free_level,
                )
                points = 0 if historical_reward else int(base_points)
                transferred_reward = historical_reward
                legacy_reward_slot = {
            ...
            points, transferred_reward = claim_free_slot_points(
                player["id"], info, base_points, payload.puzzle_id,
            )

    if ATOMIC_RESULT_V1_ENABLED:
        if atomic_identity is None:
            raise HTTPException(503, "Atomické ukládání výsledku není připravené")
        completed_at = payload_completed_at(payload.completed_at)
        team_code = (
            team_code_for_player_at(player, completed_at)
            if rankings_v2_schema_ready()
            else None
        )
        command = result_domain.build_command(
            atomic_identity,
            payload,
            ...
            legacy_reward_slot=legacy_reward_slot,
            team_code_at_completion=team_code,
        )
        receipt = result_domain.submit_atomic(command, db_atomic_result_rpc)
        logger.info(
            "atomic_result_v1 status=committed first=%s attempt_status=%s",
            bool(receipt.get("firstCompletion")),
            str(receipt.get("attemptStatus") or "unknown"),
        )
        return atomic_result_public_response(player["id"], receipt)
