...
EXCLUSION_FILE = ROOT / "data" / "target_generation_exclusions_v334.json"
MUST_REVIEW = {"červodíra", "blockchain", "pulsar", "tensor"}
if EXCLUSION_FILE.exists():
    _decisions = json.loads(EXCLUSION_FILE.read_text(encoding="utf-8"))
    MUST_REVIEW |= {
        str(word).casefold()
        for word in _decisions.get("remove_from_target_generation", [])
        if str(word).strip()
    }
