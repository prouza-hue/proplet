...
The first draft assumed all four result-update branches had identical formatting.
The current server has two inline and two wrapped forms. Patch both shapes explicitly,
then let the one-shot release checks validate the resulting server source.
"""
