...
const versionSource=fs.readFileSync(path.join(root,'proplet_version.py'),'utf8');
const expectedVersion=versionSource.match(/^APP_VERSION = "([^"]+)"/m)?.[1];
if(!expectedVersion)throw new Error('Canonical APP_VERSION missing');
...
if(preview.version!==expectedVersion||preview.gen4CandidatePreview!==true)throw new Error('Preview runtime metadata mismatch');
if(production.version!==expectedVersion||production.gen4CandidatePreview!==false)throw new Error('Production runtime metadata mismatch');
