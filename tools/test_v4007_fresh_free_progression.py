...
progression = (root / "public" / "app/content/progression.js").read_text(encoding="utf-8")
runtime_source = app + "\n" + progression
...
assert "done=slots.actual.size" in runtime_source
assert "nextUnsolved=list.find(p=>!slots.actual.has" in runtime_source
assert "unplayed=list.filter(p=>!slots.actual.has" in runtime_source
...
print("Proplet v4.01.7 fresh Gen4 progression and legacy-board detail: OK")
